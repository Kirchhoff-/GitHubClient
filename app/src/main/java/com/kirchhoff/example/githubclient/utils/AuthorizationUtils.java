...
public final class AuthorizationUtils {
    ...
    @NonNull
    public static String createAuthorizationString(@NonNull String login,
                                                   @NonNull String password) {
        String combinedStr = String.format("%1$s:%2$s", login, password);
        String authorizationString = BASIC_AUTHORIZATION +
                Base64.encodeToString(combinedStr.getBytes(),
                        Base64.DEFAULT);
